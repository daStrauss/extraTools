...
if strcmpi(renderer,'painters')
  ...
  drawnow;
  % get(p.Results.handle,'Units')
  % get(p.Results.handle,'PaperPosition')
  % get(p.Results.handle,'Position')
  % set(p.Results.handle,'PaperPosition', [0 0 7 2])
  print(p.Results.handle,'-depsc2','-loose',dpiswitch,'-painters', ...
        FileName);
  get(p.Results.handle,'PaperPosition')
  % print(p.Results.handle,'-depsc2',dpiswitch,'-painters',FileName);
